...
byte mac[] = { 0xA8, 0x61, 0x0A, 0xAE, 0x6E, 0x9D };  // MAC address for Ethernet shield

IPAddress serverIP(192, 168, 1, 41);  // IP address of the server
unsigned int serverPort = 4567;

IPAddress ip(192, 168, 1, 22);  // IP address of the Arduino
...
void processServerResponse() {
  ...
  if (response.startsWith("reset")) {
    ...
    if (y >= 5 && y <= 9) {
      //leds[x + (y - 5) * 5] = CRGB::Aquamarine;
      //FastLED.show();
      if (y == 5) {
        leds[x + 20] = CRGB::Magenta;
        FastLED.show();
      }
      if (y == 6) {
        leds[x + 15] = CRGB::Magenta;
        FastLED.show();
      }
      if (y == 7) {
        leds[x + 10] = CRGB::Magenta;
        FastLED.show();
      }
      if (y == 8) {
        leds[x + 5] = CRGB::Magenta;
        FastLED.show();
      }
      if (y == 9) {
        leds[x] = CRGB::Magenta;
